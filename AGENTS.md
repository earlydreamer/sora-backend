# 에이전트 작업 가이드 — sora-backend

이 저장소에서 Claude와 Codex가 작업할 때 지켜야 하는 정책을 정의한다.
공통 정책 섹션은 sora-frontend의 AGENTS.md와 동일하게 유지한다.
...
| 에이전트 | 담당 |
|---|---|
| **Claude** | 방향 결정, 범위 조정, 아키텍처 검토, 정책 수립, gstack 기획 스킬 실행 |
| **Codex** | 구현, 반복 수정, 테스트 작성, 디버깅, 단순 리팩토링 |
...
현재 환경에서 사용 가능한 위임 수단 (우선순위 순):

| 순위 | 수단 | 사용 시점 |
|---|---|---|
| 1 | **tmux spawn-worker** (`spawn-worker --auto-start`) | `HAS_TMUX_ORCHESTRATION=1`일 때 최우선. `spawn-worker`가 tmux window를 만들고 `tmux send-keys`로 실제 Codex CLI / Claude CLI 명령을 주입한다. 실행 상태는 `.orchestrator/` JSON과 tmux window에서 직접 확인 가능. |
| 2 | **codex-plugin-cc** (`/codex:rescue`) | `HAS_CODEX_PLUGIN=1`이고 tmux 세션이 없을 때. 로컬 `codex` 바이너리를 app server 모드로 실행 — Claude 토큰 미소비. `--background`로 비동기 실행 후 `/codex:status`, `/codex:result`로 추적. `--resume`으로 이전 세션 재개 가능. |
| 3 | **Claude Code 서브에이전트** (`Agent` 툴) | tmux도 없고 `HAS_CODEX_PLUGIN=0`일 때만 사용. Claude가 스펙 작성 후 `general-purpose` 에이전트 실행. |
| 4 | **Codex CLI (별도 터미널)** | 세션 외부 위임. Claude가 `docs/tasks/` 스펙 파일 작성 → 사람이 Codex에 전달. |

**위임 원칙**: 실제 Codex CLI 프로세스(tmux 또는 /codex:rescue)를 Agent 툴보다 항상 우선한다. "토큰이 비싼 Claude 판단은 Claude, 반복 구현은 실제 CLI 프로세스"가 핵심이다. Agent 툴은 tmux도 플러그인도 없는 절대 fallback이다.

**codex-plugin-cc 설치 방법** (1회):
```
...
/codex:setup
```
설치 확인: `/codex:setup` 출력에서 Codex ready 확인. Stop hook(review gate)는 Claude/Codex 루프 위험으로 비활성화 상태 유지.

**probe 변수**: `HAS_CODEX_PLUGIN` — probe.sh가 codex 플러그인 설치 여부를 감지해 설정.
...
- 이 저장소의 작업 완료 기준은 GitHub 정리만이 아니라 **로컬 작업 디렉터리 정렬까지 포함**한다. PR merge, issue close, 원격 브랜치 삭제 후에는 반드시 사용자가 계속 볼 로컬 작업 디렉터리를 최종 기준 브랜치로 스위치하고 최신화해야 한다.
- 여러 LLM(Claude, Codex, Gemini 등) 사이 handoff가 목적이면, 원격 상태보다 로컬 작업 디렉터리 상태가 우선 진실 원천이다. 다음 에이전트가 같은 로컬 경로를 열었을 때 바로 이어받을 수 있어야 작업이 완료된 것으로 본다.
...
```md
## 요약
- GitHub issue/PR 기반 작업 파이프라인을 backend 문서와 skill에 반영했다.

## 검증
- [x] `npm run build`
- [x] `npm run lint`
- [x] `CI=1 npm test -- --runInBand`
- [x] `CI=1 npm run test:e2e -- --runInBand`
...
## 프로젝트 상태
<현재 단계를 한 줄로> (예: 백엔드 scaffold 완료, DB 연결 대기 중)
...
예: docs/history/2026-03-30-backend-scaffold.md
```
...
### 5-1. 절대 금지
- `.env` 파일 커밋
- `node_modules/`, `dist/`, `generated/`, `.next/`, `out/` 커밋
- 아키텍처 변경(모듈 신규 추가, 새 외부 API 연동, DB 스키마 구조 변경)을 Claude 승인 없이 진행
...
### 5-5. 리뷰 시 환경 차이 무시 규칙
- Windows와 Linux/Unix 간 기본 인코딩, 개행 문자(CRLF/LF), chmod/실행 비트 차이처럼 운영체제 환경 차이에서만 발생하는 변경은 리뷰 지적 대상에서 제외한다.
- 리뷰는 실제 코드 경로, 런타임 동작, 타입/빌드/테스트 결과, API 계약처럼 **의미 있는 코드 변경**에만 집중한다.
- 인코딩 또는 줄바꿈 차이가 실제 파싱 오류, 빌드 실패, 런타임 오류를 일으킬 때만 예외적으로 다룬다.
...
### 기술 제약
- TMAP API 키는 절대 프론트엔드에 노출하지 않는다. 서버사이드 프록시만 허용.
- Prisma schema 변경 시 반드시 `npx prisma migrate dev`로 migration 생성 후 커밋.
- `generated/prisma/`는 `.gitignore`에 포함되어 있으며 커밋하지 않는다.

### 모듈 구조 원칙
- 신규 기능은 반드시 별도 NestJS 모듈로 분리한다.
- 모듈 간 의존성은 `imports` 배열로만 표현하고, 직접 인스턴스화하지 않는다.
- 전역으로 사용하는 서비스(Prisma)는 `@Global()` 모듈로만 공유한다.

### 연관 저장소
- 프론트엔드: [earlydreamer/sora-frontend](https://github.com/earlydreamer/sora-frontend)
