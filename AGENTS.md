...
| 수단 | 사용 시점 |
|---|---|
| **tmux spawn-worker** (`spawn-worker --auto-start`) | **tmux 세션이 살아 있을 때 최우선.** `spawn-worker`가 tmux window를 만들고 `tmux send-keys`로 실제 Codex CLI / Claude CLI 명령을 주입한다. 실행 상태는 `.orchestrator/` JSON과 tmux window에서 직접 확인 가능. |
| **Claude Code 서브에이전트** (`Agent` 툴) | tmux 세션이 없거나 `HAS_TMUX_ORCHESTRATION=0`일 때만 사용. Claude가 스펙 작성 후 `general-purpose` 에이전트 실행. |
| **Codex CLI (별도 터미널)** | 세션 외부 위임. Claude가 `docs/tasks/` 스펙 파일 작성 → 사람이 Codex에 전달. |

**tmux 우선 원칙**: `HAS_TMUX_ORCHESTRATION=1`이면 `Agent` 툴 대신 반드시 `spawn-worker --auto-start`를 통해 실제 터미널 프로세스에 위임한다. tmux 오케스트레이션은 "토큰이 비싼 Claude 판단은 Claude, 반복 구현은 실제 CLI 프로세스"를 실행 레벨에서 보장하기 위해 도입됐다. `Agent` 툴로 우회하면 이 구조가 무의미해진다.
