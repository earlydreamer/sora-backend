...
- 이 문서에서 말하는 작업 완료는 GitHub 상태 정리만 뜻하지 않는다. merge 뒤에는 사용자가 실제로 여는 로컬 작업 디렉터리가 최종 기준 브랜치를 가리키고 있어야 하며, 다음 에이전트가 같은 경로에서 즉시 이어받을 수 있어야 한다.
...
- Codex가 실제 구현을 시작하는 순간 work spec의 `**상태**`와 `docs/current.md`를 `codex-in-progress`로 갱신하고 현재 담당을 Codex로 맞춘다.
...
- PR merge, issue close, 원격 브랜치 삭제 뒤에는 로컬 작업 디렉터리를 최종 기준 브랜치로 스위치하고 최신화한다.
- 로컬 작업 디렉터리가 이전 작업 브랜치를 계속 가리키거나 stale 상태로 남아 있으면, 다른 LLM handoff 기준으로는 아직 마감되지 않은 것으로 본다.
