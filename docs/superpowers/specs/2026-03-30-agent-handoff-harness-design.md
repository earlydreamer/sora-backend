...
이 절은 사람이 전체 구조를 빠르게 이해하기 위한 시각 자료다. 실제 에이전트가 따라야 하는 강제 규칙은 이 문서의 상태 정의, 진입점 규칙, 산출물 계약 섹션을 기준으로 한다. 즉, 아래 도식은 설명용이고 규칙의 진실 원천은 본문이다.
...
- Claude는 "무엇을 해야 하는지"를 정리하고, Codex는 "어떻게 구현할지"를 실행한다.
