...
사람이 빠르게 훑는 개요와 도식은 [agent-handoff-harness-overview.md](/mnt/d/Projects/sora/sora-backend/docs/operations/agent-handoff-harness-overview.md)에 분리한다. 실제 에이전트가 따라야 하는 강제 규칙은 이 spec의 상태 정의, 진입점 규칙, 산출물 계약 섹션을 기준으로 한다.
